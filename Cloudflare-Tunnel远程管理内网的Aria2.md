...
注意此处的默认的RPC协议，随访问端的不同，可能是http或是Https，也可以在下一行的下拉菜单选，很多情况下http不可选，可能因为访问端浏览器自动加了证书变成了Https。
> ![alt text](/imgs/Cloudflare-Tunnel远程管理内网的Aria2/cf3_2024-06-10_01-19-25.png)



4. 改动如下图，RPC地址改为第一步中的第一个aria开头的子域名，端口因为前面rpc协议是https改为443(若RPC协议选http,端口则改为8080)，同时检查端口后的值是否是jsonrpc, 不是的话改回来。改完后点击重新加载Ariang.
...
综上，先能访问内网设备，通过内网设备进入Aria2控制台，再在RPC页面设置aria2相关项来控制管理。


以后访问时可按第二个域名先进路由器后台，再点进aria2 web控制台。也可浏览器直接输入第二个域名+ariang直接进入，如a.b.com/ariang。
做此纪录，以备忘。
