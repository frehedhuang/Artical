...
> 注意此处的默认的RPC协议，随访问端的不同，可能是http,Https，也可以在下一行的下拉菜单选，有些情况下http不可选不是CF Tunnel里设置的Http, 因为CF自动给它们增加了证书变成了Https。
> ![alt text](/imgs/Cloudflare-Tunnel远程管理内网的Aria2/cf3_2024-06-10_01-19-25.png)



>  4. 改动如下图，RPC地址改为第一步中的第一个子域名aria，端口因为前面rpc协议是https改为443，同时检查端口后的值是否是jsonrpc, 不是的话改回来。改完后点击重新加载Ariang.
>     注意此处，若RPC协议选http,端口则改为8080，
...
做此纪录，以备忘。
以后访问时可按第二个域名先进路由器后台，再点进aria2 web控制台。也可浏览器直接输入第二个域名+ariang直接进入，如
a.b.con/ariang。
